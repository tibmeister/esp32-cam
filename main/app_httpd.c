...
static esp_err_t stream_handler(httpd_req_t *req)
{
    ...
    while (true)
    {
        ...
        ESP_LOGD(TAG, "MJPG: %uB %ums (%.1ffps), AVG: %ums (%.1ffps)", (uint32_t)(_jpg_buf_len),
...
#endif
    else if (!strcmp(variable, "hostname"))
        strncpy(settings.hostname, value, LEN_HOSTNAME);
    else if (!strcmp(variable, "wifi_ssid"))
        strncpy(settings.wifi_ssid, value, LEN_WIFI_SSID);
    else if (!strcmp(variable, "wifi_password"))
        strncpy(settings.wifi_password, value, LEN_WIFI_PASSWORD);
#ifdef CONFIG_MDNS_ENABLED
    else if (!strcmp(variable, "mdns_instance"))
        strncpy(settings.mdns_instance, value, LEN_MDNS_INSTANCE);
#endif
    else if (!strcmp(variable, "dhcp"))
        settings.dhcp = val;
#ifdef CONFIG_SNTP_ENABLED
    else if (!strcmp(variable, "ntp_server"))
        strncpy(settings.ntp_server, value, LEN_NTP_SERVER);
    else if (!strcmp(variable, "timezone"))
    {
        strncpy(settings.timezone, value, LEN_TIMEZONE);
        setenv("TZ", settings.timezone, 1);
        tzset();
    }
#endif
    else if (!strcmp(variable, "ip"))
        settings.ip.addr = ipaddr_addr(value);
    else if (!strcmp(variable, "netmask"))
        settings.netmask.addr = ipaddr_addr(value);
    else if (!strcmp(variable, "gateway"))
        settings.gateway.addr = ipaddr_addr(value);
    else if (!strcmp(variable, "dns1"))
        settings.dns1.addr = ipaddr_addr(value);
    else if (!strcmp(variable, "dns2"))
        settings.dns2.addr = ipaddr_addr(value);
    else
    {
        res = -1;
    }
